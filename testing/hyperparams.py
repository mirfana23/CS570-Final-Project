...
def get_results(crop_method, num_crops, crop_size_method, confidence_measure, post_proc_logits, threshold):
    ...
    # change the values
    config['crop_method'] = crop_method
    config['num_crops'] = num_crops
    config['crop_size_method'] = crop_size_method
    config['confidence_measure'] = confidence_measure
    config['post_proc_logits'] = post_proc_logits
    config['threshold'] = threshold
    config['relabeled_dataset_save_path'] = f'/root/CS570-Final-Project/datasets/imgnet1k_original/{crop_method}_{num_crops}_{crop_size_method}_{confidence_measure}_{post_proc_logits}_{threshold}.json'

    # save the config
    with open('/root/CS570-Final-Project/config/hyper_param.yaml', 'w') as f:
        yaml.dump(config, f)

    # run the relabeller
    os.system(f'python {relabeller_path} --config /root/CS570-Final-Project/config/hyper_param.yaml')

    # run the eval and get its output
    os.system(f'python {eval_path} --pred_dataset_path /root/CS570-Final-Project/datasets/imgnet1k_original/{crop_method}_{num_crops}_{crop_size_method}_{confidence_measure}_{post_proc_logits}_{threshold}.json --gt_dataset_path /root/CS570-Final-Project/datasets/imgnet1k_ReaL/val.json > /root/CS570-Final-Project/testing/eval_output.txt')
